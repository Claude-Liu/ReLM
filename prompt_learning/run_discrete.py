...
def convert_examples_to_features(examples, max_seq_length, tokenizer, mask_src=False):
    features = []
    for i, example in enumerate(examples):
        src, trg = convert_examples_to_prompts(example.src, example.trg, max_seq_length // 2, tokenizer, mask_src)
        example.src = src
        example.trg = trg
...
def convert_examples_to_prompts(src, trg, max_seq_length, tokenizer, mask_src=False):
...
def main():
    ...
    if args.do_train:
        ...
        train_features = convert_examples_to_features(train_examples, args.max_seq_length, tokenizer, args.mft)
