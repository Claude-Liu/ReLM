...
def csc_convert_examples_to_features(examples, max_seq_length, tokenizer, static_mask=False, mask_mode="noerror"):
    features = []
    for i, example in enumerate(examples):
        encoded_inputs = tokenizer(example.text_a,
                                   max_length=max_seq_length,
        ...
        segment_ids = encoded_inputs["token_type_ids"]
        task_id = example.task.id
        ## static mask is suboptimal (we use dynamic mask e.g. mask in each step during fine-tuning)
        if static_mask:
            src_ids = mask_tokens(src_ids, trg_ids, tokenizer, mask_mode)## noise probability is 0.2
